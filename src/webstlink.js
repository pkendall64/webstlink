...
export default class WebStlink {
  ...
  _dispatch_callback (name, ...args) {
    const mutex = this._callback_mutex
  ...
  async attach (device, device_dbg = null) {
    ...
    try {
      const connector = new libstlink.usb.Connector(device, device_dbg)
      const stlink = new libstlink.Stlinkv2(connector, this._dbg)
      try {
  ...
  async find_mcus_by_core () {
    const cpuid = await this._stlink.get_debugreg32(CPUID_REG)
    if (cpuid === 0) {
      throw new libstlink.exceptions.Exception('Not connected to CPU')
    }
    this._dbg.verbose('CPUID:  ' + H32(cpuid))
    const partno = (0xfff & (cpuid >> 4))
    const mcus = libstlink.DEVICES.find(mcus => (mcus.part_no === partno))
    if (mcus) {
  ...
  async find_mcus_by_devid () {
    const idcode = await this._stlink.get_debugreg32(this._mcus_by_core.idcode_reg)
    this._dbg.verbose('IDCODE: ' + H32(idcode))
    const devid = (0xfff & idcode)
    const mcus = this._mcus_by_core.devices.find(mcus => (mcus.dev_id === devid))
    if (mcus) {
  ...
  fix_cpu_type (cpu_type) {
    ...
    if (cpu_type.startsWith('STM32')) {
      // change character at 10th position to 'x' where is package size code
  ...
  filter_detected_cpu (expected_cpus) {
    const cpus = []
    for (const detected_cpu of this._mcus) {
    ...
    if (cpus.length === 0) {
      const expected = expected_cpus.join(',')
      const possibilities = this._mcus.map(cpu => cpu.type).join(',')
  ...
  async find_sram_eeprom_size (pick_cpu = null) {
    ...
    const sram_sizes = this._mcus.map(mcu => mcu.sram_size)
    const eeprom_sizes = this._mcus.map(mcu => mcu.eeprom_size)
    ...
    if (this._mcus.length > 1) {
      ...
      if (diff) {
        let mcu = null
        if (pick_cpu) {
          const type = await pick_cpu(this._mcus)
  ...
  load_driver () {
    const flash_driver = this._mcus_by_devid.flash_driver
  ...
  async _unsafe_inspect_cpu (flush = false) {
    ...
    const lockup = (dhcsr & libstlink.drivers.Stm32.DHCSR_STATUS_LOCKUP_BIT) !== 0
    if (lockup) {
  ...
  async flash (addr, data) {
    if (data instanceof ArrayBuffer) {
      ...
      const new_data = new Uint8Array(data.length)
  ...
  async _unsafe_read_registers () {
    if (this._cache.has_all_registers()) {
      ...
      const registers = await this._driver.get_reg_all()
  ...
  async _unsafe_read_semihost_params (pointer, count) {
    const param_bytes = await this._driver.get_mem(pointer, count * 4)
    const view = new DataView(param_bytes.buffer)
    const params = []
  ...
  async _unsafe_read_semihost_operation () {
    const opcode = await this._unsafe_read_register('R0')
    const pointer = await this._unsafe_read_register('R1')
    const operation = { opcode }
    const opcodes = libstlink.semihosting.opcodes
    if (opcode === opcodes.SYS_OPEN) {
      const params = await this._unsafe_read_semihost_params(pointer, 3)
      const [filename_pointer, mode, filename_length] = params
      const modes = ['r', 'rb', 'r+', 'r+b', 'w', 'wb', 'w+', 'w+b', 'a', 'ab', 'a+', 'a+b']
      const filename_bytes = await this._driver.get_mem(filename_pointer, filename_length)
      ...
      const params = await this._unsafe_read_semihost_params(pointer, 1);
      [operation.handle] = params
    } else if (opcode === opcodes.SYS_WRITE) {
      const params = await this._unsafe_read_semihost_params(pointer, 3)
      const [handle, data_pointer, data_length] = params
  ...
  async handle_semihosting (syscall_handler) {
    ...
    try {
      ...
      const pc = await this._unsafe_read_register('PC')
      const addr = pc & 0xfffffffe
      const inst = await this._stlink.get_debugreg16(addr)
      if (inst !== 0xBEAB) {
        return false
      }

      // Read the syscall params and pass them to the handler
      const oper = await this._unsafe_read_semihost_operation()
      const result = syscall_handler(oper)
