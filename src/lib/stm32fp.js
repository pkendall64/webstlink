...
class Flash {
  constructor (driver, stlink, dbg, bank = 0) {
    ...
    const reg_bank = (FLASH_REG_BASE + (FLASH_REG_BASE_STEP * bank))
  ...
  async erase_pages (flash_start, erase_sizes, addr, size) {
    ...
    while (true) {
      for (const page_size of erase_sizes) {
  ...
  async wait_busy (wait_time, bargraph_msg = null) {
    ...
    while (Date.now() < end_time) {
      ...
      const status = await this._stlink.get_debugreg32(this.FLASH_SR_REG)
  ...
  async wait_for_breakpoint (wait_time) {
    ...
    const sr = await this._stlink.get_debugreg32(this.FLASH_SR_REG)
...
class Stm32FP extends Stm32 {
  async _flash_erase_all (bank = 0) {
    const flash = new Flash(this, this._stlink, this._dbg, bank)
    await flash.init()
