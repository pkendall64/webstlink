...
class Stm32 {
  ...
  get_reg (reg) {
    ...
    const index = REGISTERS.indexOf(reg.toUpperCase())
    if (index !== -1) {
  ...
  async get_mem (addr, size) {
    ...
    const chunks = []
    if (addr % 4) {
      const read_size = Math.min((4 - (addr % 4)), size)
      const chunk = await this._stlink.get_mem8(addr, read_size)
    ...
    while (true) {
      ...
      if (read_size > 64) {
        ...
        const chunk = await this._stlink.get_mem32((addr + total), read_size)
        total += chunk.byteLength
        chunks.push(chunk)
      }
    }
    if (total < size) {
      const read_size = (size - total)
      const chunk = await this._stlink.get_mem8((addr + total), read_size)
      total += chunk.byteLength
      chunks.push(chunk)
    }
    ...
    for (const chunk of chunks) {
      for (const b of new Uint8Array(chunk.buffer)) {
        data[i++] = b
      }
    }
  ...
  async set_mem (addr, data) {
    ...
    let written_size = 0
    if (addr % 4) {
      const write_size = Math.min((4 - (addr % 4)), data.length)
