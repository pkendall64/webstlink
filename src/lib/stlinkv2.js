...
export default class Stlink {
  ...
  async read_version () {
    ...
    const rx = await this._connector.xfer([STLINK_GET_VERSION, 0x80], { rx_len: 6, retry: 2 })
    const ver = rx.getUint16(0)

    const dev_ver = this._connector.version
  ...
  async read_target_voltage () {
    const rx = await this._connector.xfer([STLINK_GET_TARGET_VOLTAGE], { rx_len: 8 })
  ...
  async read_coreid () {
    const rx = await this._connector.xfer([STLINK_DEBUG_COMMAND, STLINK_DEBUG_READCOREID], { rx_len: 4 })
  ...
  async leave_state () {
    const rx = await this._connector.xfer([STLINK_GET_CURRENT_MODE], { rx_len: 2 })
  ...
  async set_swd_freq (freq = 1800000) {
    for (const [f, divisor] of STLINK_DEBUG_APIV2_SWD_SET_FREQ_MAP) {
      if (freq >= f) {
        const cmd = [STLINK_DEBUG_COMMAND, STLINK_DEBUG_APIV2_SWD_SET_FREQ, divisor]
        const rx = await this._connector.xfer(cmd, { rx_len: 2 })
  ...
  async get_debugreg32 (addr) {
    ...
    const cmd = new ArrayBuffer(6)
    const view = new DataView(cmd)
    view.setUint8(0, STLINK_DEBUG_COMMAND)
    view.setUint8(1, STLINK_DEBUG_APIV2_READDEBUGREG)
    view.setUint32(2, addr, true)
    const rx = await this._connector.xfer(cmd, { rx_len: 8 })
  ...
  async get_reg (reg) {
    const cmd = [STLINK_DEBUG_COMMAND, STLINK_DEBUG_APIV2_READREG, reg]
    const rx = await this._connector.xfer(cmd, { rx_len: 8 })
    return rx.getUint32(4, true)
  }
  ...
  set_mem32 (addr, data) {
    ...
    return this._connector.xfer(cmd, { data })
  }
