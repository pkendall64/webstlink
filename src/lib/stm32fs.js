...
class Flash {
  ...
  async get_voltage_dependend_params () {
    ...
    const voltage = this._stlink.target_voltage
    const params = VOLTAGE_DEPENDEND_PARAMS.find(
  ...
  async erase_sectors (flash_start, erase_sizes, addr, size) {
    ...
    let sector = 0
    while (true) {
      for (const erase_size of erase_sizes) {
  ...
  async wait_busy (wait_time, bargraph_msg = null) {
    ...
    while (Date.now() < end_time) {
      ...
      const status = await this._stlink.get_debugreg32(FLASH_SR_REG)
  ...
  async wait_for_breakpoint (wait_time) {
    ...
    const sr = await this._stlink.get_debugreg32(FLASH_SR_REG)
...
class Stm32FS extends Stm32 {
  async flash_erase_all () {
    ...
    const flash = new Flash(this, this._stlink, this._dbg)
    await flash.init()
  ...
  async flash_write (addr, data, { erase = false, verify = false, erase_sizes = null }) {
    const addr_str = (addr !== null) ? `0x${H32(addr)}` : 'None'
    ...
    // align data
    if (data.length % 4) {
      const padded_data = new Uint8Array(data.length + (4 - (data.length % 4)))
    ...
    while (data.length > 0) {
      ...
      const block = data.slice(0, this._stlink.maximum_transfer_size)
      data = data.slice(this._stlink.maximum_transfer_size)
      await flash.write(addr, block)
      if (verify) {
        const flashed_data = await this._stlink.get_mem32(addr, block.length)
        const flashed_block = new Uint8Array(flashed_data.buffer)
        let verified = false
