...
const Connector = class StlinkUsbConnector {
  constructor (dev, dbg = null) {
    this._dbg = dbg
    this._dev = null
    this._dev_type = null
    for (const dev_type of DEV_TYPES) {
  ...
  async connect () {
    ...
    const intf = this._dev.configuration.interfaces[0]
    if (!intf.claimed) {
  ...
  async xfer (cmd, { data = null, rx_len = null, retry = 0 } = {}) {
    ...
    // pad to 16 bytes
    const cmd_buffer = new Uint8Array(STLINK_CMD_SIZE_V2)
    ...
    while (true) {
      try {
        ...
        if (rx_len) {
          const rx = await this._read(rx_len)
          return rx
        }
      } catch (e) {
